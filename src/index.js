...
class KadDHT extends EventEmitter {
  ...
  async put (key, value, options = {}) {
    ...
    const results = pFilter(peers, async (peer) => {
      try {
    ...
    const minPeers = options.minPeers || peers.length // Ensure we have a default `minPeers` TODO: here
  ...
  async getClosestPeers (key, options = { shallow: false }) {
    ...
    const sorted = utils.sortClosestPeers(Array.from(res.finalSet), id)
  ...
  async provide (key) {
    ...
    const peers = this.getClosestPeers(key.buffer)
  ...
  async findPeer (id, options = {}) {
    ...
    // Try to find locally
    const pi = await this.findPeerLocal()
    ...
    const query = new Query(this, id.id, () => {
      ...
      // Just return the actual query function.
      return async (peer) => {
        const msg = await this._findPeerSingleAsync(peer, id) // TODO: change
